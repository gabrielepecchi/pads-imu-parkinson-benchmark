...
![Per-fold metrics strip chart](results/per_fold_metrics.png)

*Each dot is one test fold; the horizontal bar is the mean across five folds. LR and CNN show tight, consistent variance across all metrics. RF shows near-perfect sensitivity but near-zero specificity at the default decision threshold — it classifies almost every sample as PD.*

![Sensitivity vs specificity per model](results/sensitivity_specificity.png)

*Mean ± std across five folds. Random Forest achieves high sensitivity (0.99) but collapses to near-zero specificity (0.07) at the default threshold, indicating a calibration issue rather than strong binary classification. Logistic Regression achieves the most balanced trade-off. Figures generated by `plot_results.py` from `results/per_fold_metrics.csv` and `results/metrics_summary.csv`.*

> **Note on ROC curves:** per-sample predicted probabilities are not persisted by the current pipeline, so fold-level ROC curves cannot be reconstructed post-hoc. AUROC is reported as a scalar computed within each fold during the pipeline run. Saving probabilities to enable ROC curve plotting is listed under Future Improvements.
...
* [ ] Save per-sample predicted probabilities per fold to enable post-hoc ROC curve plotting.
